                ...
                <TransitionChild as="template" enter="ease-out duration-300"
                    ...
                    <div
                        ...
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="sm:flex sm:items-start">
                                ...
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    ...
                                    <div class="pt-6">
                                        <select v-model="voteType" id="voteType" name="voteType"
                                    ...
                                    <div v-if="voteType === 'change'">
                                        ...
                                        <div v-if="stateKey && !stateKeys.find(key => key.id === stateKey).valid">
                                            <input :type="computeStateValueType" v-model="stateValue"
                                                @keyup="recalcFields" placeholder="Value"
                                                :class="inputBox(dataValid)" />
                                        ...
                                        <div v-else-if="stateKey">
                                            <select v-model="stateValue" @change="recalcFields" id="stateValue"
                                                name="stateValue"
                                                class="mt-1 block w-3/4 pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                                                <option value="" disabled selected>Value</option>
                                                <option
                                                    v-for="value in stateKeys.find(key => key.id === stateKey).valid.filter(value => value.toLowerCase() !== repo[stateKey.toLowerCase()].toLowerCase())"
                                    ...
                                    <div v-else-if="voteType === 'member'">
                                        <div class="col-span-2 mt-2 mb-2">
                                            <input type="radio" v-model="memberVoteType" @change="refreshMember"
                                                id="add" value="add" class="form-radio text-aftrBlue"><label
                                                class="px-2 text-sm text-gray-700">Add</label>
                                            <input type="radio" v-model="memberVoteType" @change="refreshMember"
                                                id="remove" value="remove" class="form-radio text-aftrBlue"><label
                                                class="px-2 text-sm text-gray-700">Remove</label>
                                        </div>
                                        <div v-if="memberVoteType === 'add'" class="flex flex-col">
                                            <input type="text" v-model="memberWallet" :placeholder="memberWalletText"
                                                @keyup="recalcFields" :class="inputBox(dataValid)" />
                                            <input type="number" v-model="memberQty" placeholder="# of Tokens to Mint"
                                                @keyup="recalcFields" :class="inputBox(dataValid)" />
                                        </div>
                                        <div v-else class="flex flex-col gap-y-2">
                                            <select v-model="memberWallet" @change="changeMember" id="memberWallet"
                                                name="memberWallet"
                                                class="mt-1 w-3/4 block pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                                                <option value="" disabled selected>Choose Member to Remove</option>
                                                <option v-for="(tokens, addr) in repo.balances" :key=addr :value=addr>
                                                    {{ addr }}
                                                </option>
                                            </select>
                                        </div>
                                        <div v-if="memberQty !== ''" class="pl-2 text-xs text-aftrDarkGrey">
                                            <p>
                                                ...
                                                <span class="text-aftrBlue">{{ walletAddressSubstr(memberWallet)
                                                }}</span>
                                            </p>
                                            <p>
                                    ...
                                    <div v-else-if="voteType === 'token'">
                                        <div class="col-span-2 mt-2 mb-2">
                                            <input type="radio" v-model="tokenAction" @change="refreshMember" id="mint"
                                                value="mint" class="form-radio text-aftrBlue"><label
                                                class="px-2 text-sm text-gray-700">Mint</label>
                                            <input type="radio" v-model="tokenAction" @change="refreshMember" id="burn"
                                                value="burn" class="form-radio text-aftrBlue"><label
                                                class="px-2 text-sm text-gray-700">Burn</label>
                                        </div>
                                        <div class="flex flex-col">
                                            <select v-model="memberWallet" id="memberWallet" name="memberWallet"
                                                @change="changeMember"
                                                class="mt-1 w-3/4 block pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                                            ...
                                            <input type="number" v-model="memberQty" :placeholder="tokenQtyText"
                                                :class="inputBox(dataValid)" @keyup="recalcFields" />
                                            <div v-if="memberQty !== ''" class="pl-2 text-xs text-aftrDarkGrey">
                                                <p>
                                    ...
                                    <div class="mt-8">
                                        ...
                                        <p v-else class="text-sm text-aftrRed">
                                            Fill out repo settings to propose a vote.
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                            ...
                            <button v-if="dataValid" @click="proposeVote" type="button"
...
export default {
    ...
    methods: {
        ...
        changeKey() {
            ...
            } else if (typeof this.repo[keyString] !== 'undefined') {
                newValue = this.repo[keyString];
            } else {
                newValue = '';
            }
        ...
        recalcFields() {
            ...
            if (this.voteType === 'change') {
                ...
                let newTotal = 0;
                if (this.tokenAction === 'mint') {
                    newTotal = Number(this.repo.balances[this.memberWallet]) + Number(this.memberQty);
                } else {
                    newTotal = Number(this.repo.balances[this.memberWallet]) - Number(this.memberQty);
                    if (newTotal < 0) {
        ...
        changeMember() {
            ...
            this.memberQty = this.repo.balances[this.memberWallet];
            this.recalcFields();
        },
        proposeVote() {
            ...
            ***/
            ...
            this.$log.info("RepoVotesAdd : proposeVote :: ", "CONTRACT:  " + this.repo.id);
        }
    },
    setup() {
        const open = ref(true)
