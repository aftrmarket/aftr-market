<template>
    ...
    <perfect-scrollbar>
        <div :class="editClass">
            <!-- Setting Table -->
            <div v-if="uiEditMode || settingArray.length > 0"
                class="shadow col-span-3 overflow-hidden border-b border-gray-200 sm:rounded-lg">
                    ...
                    <thead class="bg-gray-50">
                        <tr>
                            <th scope="col"
                                ...
                                <button v-if="uiEditMode" @click.prevent="addSettingRow" type="button"
                    ...
                    <tbody class="bg-white divide-y divide-gray-200" v-for="val in settingArray" :key="val">
                        <tr v-for="v in val[1]" :key="v" class="text-xs text-gray-500 hover:bg-gray-50">
                            ...
                            <td v-if="uiEditMode" class="text-right px-4 py-2">
                                <textarea v-model="memberUpdates[v.value, v.name]" @blur="onDirty"
                            ...
                            <td class="text-right px-4 py-2" v-if="uiEditMode">
                                <input type="number" v-model="memberPriorityUpdates[v.priority, v.name]"
                                    @blur="updatePriority"
                                    class="mt-1 mb-1 mr-4 w-36 text-xs text-right focus:ring-aftrBlue focus:border-aftrBlue shadow-sm border-gray-300 rounded-md" />
                            </td>
                            <!-- <td v-if="uiEditMode" class="text-right px-4 py-2">
                            ...
                            <td v-if="uiEditMode" class="text-center px-4 py-2">
                                <button @click.prevent="removeSetting(val, v)" type="button"
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-show="addRow" class="text-xs text-gray-500 hover:bg-gray-50">
                            <td class="text-left px-2 py-2">
                                <input type="text" v-model="newSettingKey"
                                    class="mt-1 mb-1 w-full text-xs focus:ring-aftrBlue focus:border-aftrBlue shadow-sm border-gray-300 rounded-md">
                            </td>
                            <td class="text-right px-4 py-2">
                                <textarea v-model="newSettingValue" @blur="onDirty"
                                    class="mt-1 mb-1 mr-4 w-36 text-xs text-right focus:ring-aftrBlue focus:border-aftrBlue shadow-sm border-gray-300 rounded-md" />
                            </td>
                            <td class="text-right px-4 py-2">
                                <input type="number" v-model="newSettingPriority" @blur="onDirty"
                                    class="mt-1 mb-1 mr-4 w-36 text-xs text-right focus:ring-aftrBlue focus:border-aftrBlue shadow-sm border-gray-300 rounded-md" />
                            </td>
                            <td class="text-center px-4 py-2">
                                <button @click.prevent="addSetting" type="button"
            ...
            <div v-if="uiEditMode && numChanges > 0"
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr v-for="(value, index) in settingAdds" :key="value"
                            class="text-xs text-gray-500 hover:bg-gray-50">
                            <td class="px-4 py-2">
                            ...
                            <td class="px-4 py-2 text-center">
                                <button @click.prevent="removeProposal(value[0], 'add')" type="button"
                                    class="inline-flex items-center px-1 py-1 border border-transparent shadow-sm text-sm font-medium rounded-md text-aftrRed bg-white hover:bg-aftrRed hover:text-white focus:outline-none">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20"
                                        fill="currentColor">
                                        <path fill-rule="evenodd"
                        ...
                        <tr v-for="(value, index) in settingRemoves" :key="value"
                            class="text-xs text-gray-500 hover:bg-gray-50">
                            <td class="px-4 py-2">
                            ...
                            <td class="px-4 py-2 text-center">
                                <button @click.prevent="removeProposal(value, 'remove')" type="button"
                                    class="inline-flex items-center px-1 py-1 border border-transparent shadow-sm text-sm font-medium rounded-md text-aftrRed bg-white hover:bg-aftrRed hover:text-white focus:outline-none">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20"
                                        fill="currentColor">
                                        <path fill-rule="evenodd"
                        ...
                        <tr v-for="(key, index) in Object.keys(memberPriorityUpdates)" :key="key"
                            class="text-xs text-gray-500 hover:bg-gray-50">
                            <td class="px-4 py-2">
                                {{ index + 1 }}
                            </td>
                            ...
                            <td class="px-4 py-2 text-center">
                                <button @click.prevent="removeProposal(key, 'updatePriority')" type="button"
                                    class="inline-flex items-center px-1 py-1 border border-transparent shadow-sm text-sm font-medium rounded-md text-aftrRed bg-white hover:bg-aftrRed hover:text-white focus:outline-none">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20"
                                        fill="currentColor">
                                        <path fill-rule="evenodd"
...
export default {
    ...
    mounted() {
        this.repo.settings.map((value, index) => {
    ...
    computed: {
        ...
        statusMessage() {
            if (this.getActiveAddress === this.ownerAddress && this.repo.ownership === 'single') {
                return "Settings changes will process immediately because you are the owner of the repo";
            } else {
                return "Settings changes will be proposed as votes because this is a multiple owner repo";
            }
        },
    ...
    methods: {
        updatePriority() {
            ...
            console.log(this.repo.settings)
        ...
        onDirty() {
            ...
            for (let member in this.memberUpdates) {
                ...
                console.log("this.repo.settings", this.repo.settings)
            }
        ...
        buildInput(settingKey, settingValue, type = 'tokenChange') {
            let input = {};
            // let currentsettingValue = +this.repo.balances[settingKey];
        ...
        async submit() {
            ...
            if (count === 1) {
                if (this.settingRemoves.length === 1) {
                    ...
                    settingKey = "names";
                    this.repo.settings.map((item, index) => {
                        if (item[0] == "names") {
            ...
            this.$log.info("RepoSettings : submit :: ", JSON.stringify(input));
            this.$swal({
                icon: "info",
            ...
            let txid = await warpWrite(this.repo.id, input);
            this.$log.info("RepoSettings : sumbit :: ", "TX: " + txid);
            this.$swal.close();
