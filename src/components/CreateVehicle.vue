    ...
    <div v-if="pageStatus === ''">
      ...
      <form-container>
        <form action="#" method="POST">
          ...
          <div class="bg-white sm:p-6">
            ...
            <div class="pt-2 grid grid-cols-3 gap-4">
              ...
              <input
                ...
                ref="vehicleTicker"
                @input="tickerValidate"
                :class="inputBox(tickerValid)"
              />
            </div>
            <div class="pt-2 grid grid-cols-3 gap-4">
            ...
            <div>
              <label
                for="vehicleLogo"
                class="py-2 block text-sm font-medium text-gray-700"
                >Logo</label
              >
              <div class="mt-2 sm:mt-0 sm:col-span-2 pl-6">
                ...
                <p
                  class="text-xs text-aftrRed"
                  v-if="totalSize == 0 && totalCost == 0"
                >
                  AR fees apply
                </p>
                <p
          ...
          <h3
            class="mt-4 border-t border-gray-200 pt-4 text-xl font-light leading-6"
          >
            Settings
          </h3>
          <div class="bg-white sm:p-6">
            <div class="pt-2 grid grid-cols-4 flex items-center gap-x-4">
              ...
              >
              <label class="block text-sm font-medium text-gray-700"
                >Quorum (between 0.01 - 0.99)</label
              >
              <label class="block text-sm font-medium text-gray-700"
                >Support(between 0.01 - 0.99)</label
              >
              ...
              <div class="">
                ...
                /><label class="px-2 text-sm text-gray-700">Single Owner</label>
                <input
                  type="radio"
                  v-model="ownership"
                  id="dao"
                  value="dao"
                  class="form-radio text-aftrBlue"
                /><label class="px-2 text-sm text-gray-700">DAO Owned</label>
              </div>
              <div class="">
                <input
                  type="radio"
          ...
          <div class="bg-white sm:p-6">
            ...
            <div v-if="arConnected" class="pt-6">
              ...
              >
                ...
                <option
                  v-for="pst in $store.getters.getActiveWallet.psts"
                  :key="pst.id"
                  :value="pst.id"
                >
                  {{ pst.name }} ({{ pst.id }})
            ...
            <div v-if="selectedPstId !== ''">
              ...
              <input
                type="number"
                placeholder="Amount"
                v-model="pstInputTokens"
                @input="calcPstPrice"
                :class="inputBox(pstInputValid)"
              />
              <span v-if="pstInputTokens" class="pl-4 pr-6"
                >@ {{ formatNumber(pricePerToken, true) }} AR
                {{
                  pstInputTokens
              ...
              <div class="pt-2">
                <button
                  v-if="pstInputValid && pstInputTokens"
                  @click.prevent="addPst"
                  type="submit"
                      ...
                      <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        >
                            ...
                            <div class="flex items-center">
                              <div class="flex-shrink-0 h-10 w-10">
                                <img
                                  ...
                                  :src="`https://arweave.net/` + pst.logo"
                                  alt=""
                                />
                              </div>
                              <div class="ml-4">
                          ...
                          <td class="text-center px-6 py-3">
                            <button
                              @click.prevent="removePst(pst.id)"
                              type="submit"
          ...
          <div class="bg-white sm:p-4">
            <div class="flex items-stretch">
              <div class="mt-6 flow-root">
                <input
                  type="text"
                  v-model="memberWallet"
                  placeholder="Wallet Address"
                  @input="onTokenChange"
                  :class="inputBox(memberWalletValid)"
                  class="w-96"
                />
              </div>
              <div class="mt-6 pl-4 flex flex-col inline-flex">
                <input
                  ...
                  @input="onTokenChange"
                  :class="inputBox(memberAmountValid)"
                />
              </div>
              <div class="mt-6 pl-4 flex flex-col inline-flex">
                <div v-if="!memberAmountValid">
                  <label
                    class="pl-4 flex flex-col inline-flex block text-sm text-aftrRed"
                  >
                ...
                <button
                  v-if="memberWalletValid && memberAmountValid"
                  @click.prevent="addDaoMember"
                  type="button"
                ...
                >
                  ...
                  >
                    <table class="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody class="bg-white divide-y divide-gray-200">
                        <tr
                          v-for="(member, index) in daoMembers"
                          :key="index"
                          class="hover:bg-gray-50"
                        >
                          ...
                          <td class="flex content-center gap-x-2">
                            <input
                              ...
                              v-model="daoRowBalance[index]"
                              class="mt-1 mb-1 mr-4 focus:ring-aftrBlue focus:border-aftrBlue shadow-sm sm:text-sm border-gray-300 rounded-md"
                            />
                            <button
                              @click.prevent="daoRowChange(member.wallet)"
                              type="submit"
                          ...
                          <td v-else class="text-center px-6 py-3">
                            <button
                              @click.prevent="removeDaoMember(member)"
                              type="submit"
                              class="inline-flex items-center p-1 border border-transparent shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-aftrRed"
                            >
                              <img src="../assets/delete-24px.svg" />
          ...
          <!-- Button Row --->
          <div
            class="px-4 py-3 bg-gray-50 sm:px-6 flex items-center justify-between"
          >
            <button
              type="submit"
...
export default {
  ...
  methods: {
    ...
    addPst() {
      ...
      const newBal =
        this.$store.getters.getActiveWallet.psts[existingIndex]["balance"] -
    ...
    removePst(id) {
      ...
      const existingIndex = this.$store.getters.getActiveWallet.psts.findIndex(
        (item) => item.id === id
      );
      const newBal =
        this.$store.getters.getActiveWallet.psts[existingIndex].balance +
