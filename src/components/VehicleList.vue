      ...
      <div class="max-w-7xl mx-auto pb-12 px-4 sm:px-6 lg:px-8">
        ...
        <div class="pb-5 sm:flex sm:items-center sm:justify-between">
          <select
            ...
            class="text-blue"
            @change="getVehicle($event)"
          >
            <option value="" disabled selected>Select Vehicles</option>
            <option v-bind:value="all_vehicle">All Vehicle</option>
            <option v-bind:value="my_vehicle">My Vehicle</option>
            <option
        ...
        <div class="bg-white rounded-lg shadow px-5 py-6 sm:px-6">
          ...
          >
            <li
              v-for="vehicle in selectedVehicle"
              :key="vehicle.id"
              class="col-span-1 bg-white rounded-lg shadow divide-gray-200"
            >
              <router-link
                :to="{ name: 'vehicle', params: { vehicleId: vehicle.id } }"
              >
          ...
          <ul
            v-else-if="getVehicleList"
            class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3"
          >
            <li
              v-for="vehicle in selectedMyVehicle"
              :key="vehicle.id"
              class="col-span-1 bg-white rounded-lg shadow divide-gray-200"
            >
              <router-link
      ...
      try {
        ...
        for (let token of vehicle.tokens) {
          try {
            ...
            console.log(
              "ERROR calling Verto cache on " + token.name + ": " + error
            );
          }
        }
