...
export default {
    ...
    async created() {
        if (import.meta.env.VITE_ENV === "PROD" || import.meta.env.VITE_ENV === "TEST") {
            ...
            let aftrContractSourceId = csArray[csArray.length - 1]
            // get a contract that uses this source
            let route = 'https://gateway.redstone.finance/gateway/contracts-by-source?id=' + aftrContractSourceId + (this.network === 'TEST' ? '&testnet=true' : '');
            let response = await fetch(route)
            let data = await response.json()
            let contract = data.contracts[0]
            let contractId = contract.contractId

            route = 'https://gateway.redstone.finance/gateway/contract?txId=' + contractId + (this.network === 'TEST' ? '&testnet=true' : '');
            response = await fetch(route)
